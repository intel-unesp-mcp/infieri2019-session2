...
extern "C" void sgemm_fortran(float*, float*, float*, FTI_POINTER(int), FTI_POINTER(int), FTI_POINTER(int));
extern "C" void dgemm_fortran(double*, double*, double*, FTI_POINTER(int), FTI_POINTER(int), FTI_POINTER(int));
...
TARGET(mic) void gemm(T* result, const T* a, const T* b, std::size_t arows, std::size_t acols, std::size_t bcols)
{
  struct TARGET(mic) local {
    ...
    static void gemm(float* result, float* a, float* b, int arows, int acols, int bcols) {
      sgemm_fortran(result, a, b, FTI_ADDRESS(arows), FTI_ADDRESS(acols), FTI_ADDRESS(bcols));
    }

    static void gemm(double* result, double* a, double* b, int arows, int acols, int bcols) {
      dgemm_fortran(result, a, b, FTI_ADDRESS(arows), FTI_ADDRESS(acols), FTI_ADDRESS(bcols));
    }
#endif
  };
